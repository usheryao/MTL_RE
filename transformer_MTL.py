...
pcnn_dir=r'../HNRE/outputs/ckpt/pcnn_nnatt/lr_0.2/27/pcnn-54960'
pcnn_reader = pywrap_tensorflow.NewCheckpointReader(pcnn_dir)
...
class Transformer():
    ...
    def bulid(self,init_vec):
        ...
        with tf.variable_scope("embedding-lookup", initializer=xavier(), dtype=tf.float32):
            ...
            temp_word_embedding = tf.get_variable(name = 'temp_word_embedding', initializer = pcnn_reader.get_tensor('embedding-lookup/temp_word_embedding'), trainable = True)
            unk_word_embedding = tf.get_variable(name = 'unk_embedding', initializer = pcnn_reader.get_tensor('embedding-lookup/unk_embedding'), trainable = True)
            ...
            temp_pos1_embedding = tf.get_variable(name = 'temp_pos1_embedding', initializer = pcnn_reader.get_tensor('embedding-lookup/temp_pos1_embedding'), trainable = True)
            temp_pos2_embedding = tf.get_variable(name = 'temp_pos2_embedding', initializer = pcnn_reader.get_tensor('embedding-lookup/temp_pos2_embedding'), trainable = True)
